...
async function getHistory(sym, periodo, stl) {
	...
	ws.onmessage = function (msg) {
      ...
      const cleaned = history.map(candle => ({
        epoch: candle.epoch,
        close: candle.close,
      }))
...
async function run() {
  ...
  const data = await getHistory('frxAUDCAD', 300, 'candles');
  

  // More code will be added below
}

async function continue_(data){
  console.log(data);
